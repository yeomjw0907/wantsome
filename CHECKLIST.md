...
- PortOne 계정 생성 ([https://portone.io](https://portone.io)) → PASS 본인인증 채널 신청
...
- Apple Developer Program 가입 ($99/년) — [https://developer.apple.com](https://developer.apple.com)
...
- Google Play Console 계정 ($25 1회) — [https://play.google.com/console](https://play.google.com/console)
...
- Google Cloud Console ([https://console.cloud.google.com](https://console.cloud.google.com)) → OAuth 2.0 클라이언트 ID 생성
...
- Kakao Developers ([https://developers.kakao.com](https://developers.kakao.com)) → 앱 등록
...
- Bundle ID: `kr.wantsome.app` (iOS & Android)
- Expo SDK 55 + 모든 플러그인 설정
- Supabase 프로젝트 설정 (ftnfdtvaxsvosdyjdxfq)
- Agora App ID + Certificate 설정
- 앱 아이콘 / 스플래시 / 알림 아이콘
- 화면 녹화 방지 (expo-screen-capture)
- wantsome.kr 도메인 확보
- 주요 기능 구현 (피드, 화상통화, 예약, 쇼핑, 선물, 채팅)
- 어드민 패널 기본 구조 (대시보드, 크리에이터, 유저, 리포트, 정산)
- DB 스키마 완성 (migrations 001~011)
- 더미 데이터 (크리에이터 5명, 포스트 8개, 상품 8개)
- 테스트 우회 버튼 `__DEV__` 처리 완료
- 개인정보처리방침/이용약관 스켈레톤 페이지 생성
