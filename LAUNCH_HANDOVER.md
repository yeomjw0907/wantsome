...
# ═══ DAY 3 UPDATE (2026-05-27) ═══

## 오늘 끝낸 것
- **사업자정보 노출 코드 작업 완료** (commit `4c7a958`, Vercel READY)
  - `constants/businessInfo.ts`, `server/lib/businessInfo.ts` 상수 신규
  - `app/(app)/settings/business-info.tsx` 앱 화면 + settings 메뉴에 진입 항목
  - `server/app/business/page.tsx` 신규 `/business` 공개 URL
  - terms/privacy 페이지 푸터에 5종 + `/business` 링크
  - root page 사업자 섹션 fallback을 상수 값으로
- **라이브 사업자정보 페이지**: https://api.wantsome.kr/business — 5종 + 유선(032-279-4649) + 이메일(cs@wantsome.kr) 정상 노출 확인
- **공기계(Android) 도착** — 향후 ① 통화·라이브 화면 캡처(2계정 스테이징) ② Android Play 14일 테스트용
- **다날 1차 회신 완료** (2026-05-27 오전)
  - 첨부: `DI_CI_제공신청서_원썸_채움.docx`(법인 인감 PDF) + `본인확인프로세스_원썸_초안.pptx`(슬라이드 3 = 라이브 페이지 캡처)
  - 본문: 가격제 추천안 요청 (월정액/건당/구간별 중)
- **다날 2차 회신 (이지호)** — 수정 요청 도착:
  1. **DICI 신청서**: 담당자 정보 **마스킹** 필요 / 서비스명·URL **"원썸(wantsome) / APP"** 으로만
  2. **본인확인프로세스 PPTX**: 사용절차도 **실제 화면 캡처** 로 작성 (슬라이드 3 사업자정보는 OK)
  3. **요금제**: 정액제만 운영 → **A~D형 중 선택** (단가표 미첨부 — 회신 요청 필요)
- **DICI 신청서 수정본 즉시 작성 완료** (담당자=마스킹, URL=원썸(wantsome) / APP)

## 내일(DAY 4) 바로 할 일
1. **앱 핸드폰 캡처 3장** → `C:\dev\wantsome\danal\` 에 넣기
   - `절차1_약관.png` (회원가입 약관 동의 화면)
   - `절차2_연령확인.png` (생년월일 입력 — 어제 거 재활용 가능)
   - `절차3_완료.png` (인증 후 프로필 작성 or 홈)
2. **PPTX 슬라이드 2 교체** — 위 3컷 흐름 + 절차 설명
   - 코멘트로 "현재 PortOne 활성화 전 fallback 단계 / 계약 완료 후 PASS 본인확인으로 전환" 명시
3. **다날 2차 회신 메일** 보내기
   - 첨부: 수정 DI/CI 신청서 PDF + 수정 PPTX
   - 본문: **A~D형 정액제 단가표(부칙안) 회신 요청** 추가
4. (병행) ASC 심사 제출 작업 — 앱 스크린샷 4장 가공 → ASC 업로드, 등록정보 작성, IAP 첨부, Privacy Label, 빌드 선택, 제출

## 현재 파일 상태 (gitignored: `danal/` 폴더 전체)
- `danal/DI_CI_제공신청서_원썸_채움.docx` — **수정본** (마스킹/URL 반영)
- `danal/본인확인서비스_이용계약서_원썸_채움.docx` — 담당자 채움. 다날 가격제(A~D) 받고 "신청 서비스 구분" 체크 후 출력→인감→등기
