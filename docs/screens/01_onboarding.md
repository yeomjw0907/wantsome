...
```
splash → login → terms → verify → role → (mode | creator_onboarding) → profile → charge_promo → 메인
```
...
| 모드 선택 | (auth)/mode.tsx | 빨간불 선택 시 추가 동의 팝업 |
...
- app/(auth)/mode.tsx          → 파란불/빨간불 선택, 빨간불 추가 동의 BottomSheet
